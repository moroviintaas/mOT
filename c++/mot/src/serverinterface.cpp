    ...
    try
    {
        ...
        for (;;)
        {
            ...
            if(recv_buf[0]==flag_client_hello)
            {
                ...
                if(SETTING_1024_160_SHA256_SHA256 != recv_buf[FIELD_SETTING])
                {
                    std::cerr<<"Error bad settings for domain parameters, now supporting only SETTING_1024_160_SHA256_SHA256\n";
                }
                ...
                client_message = buffertompzclass(recv_buf,FIELD_MESSAGE_START+size_of_id_dest+received_size_of_id_source,size_in_mod);

                server_message = tmp_context.protocol_message_cint();
                kgc_id_in_request = buffertouint64(recv_buf,FIELD_MESSAGE_START+size_of_id_dest+received_size_of_id_source + size_in_mod + 1);

                if(kgc_id_in_request != context_template.get_kgc_id())
                {
                    std::cerr<<"Error: bad Kgc id:\nrequested:\t"<<std::hex<<kgc_id_in_request<<"\nexpected:\t"<<std::hex<<context_template.get_kgc_id()<<"\n";

                }
                else {
                    new_session_id = d(rd)<<16;
                    new_session_id ^= buffertouint32(recv_buf,FIELD_SESSION_ID);
                    ...
                    */
                    ...
                    uint32tobuffer(new_session_id, send_buf,FIELD_SESSION_ID);
                    ...
                    stringtobuffer(send_buf,context_template.get_user_id(),FIELD_MESSAGE_START);
                    stringtobuffer(send_buf, corresponder_id,FIELD_MESSAGE_START+size_of_src_id);
                    mpzclasstobuffer(server_message,send_buf,FIELD_MESSAGE_START+size_of_src_id + size_of_id_dest);
                    /*
